...
'''
    20220215
    For GetParamStats, add prob as an input with default values of [.025,.25,.5,.75,.975]
    '''
...
def GetParamValues(FileName, ParamName,burn=0,nthin=None):
    result=GetParamValues2(FileName, ParamName,burn=burn,nthin=nthin)
    return (result)
...
def GetParamStats(FileName, ParamName,burn=0,prob=[.025,.25,.5,.75,.975]):
	...
	try:
	    xquantiles=mquantiles(xarray,prob=prob)
	    #xhpd=hpd(xarray, .05)
	    return {
		'n': n,
		...
		'mc error': xarray.std(0) / np.sqrt(n)  ,
